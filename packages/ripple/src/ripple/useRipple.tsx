...
export function useRipple<T extends HTMLElement | SVGSVGElement>(
  ...
  const createRipple = useCallback(
    (x: number, y: number, rippleConfig: RippleConfig = {}) => {
      ...
      const radius =
        mergedConfig.radius || distanceToFurthestCorner(x, y, containerRect);
      ...
      setTimeout(() => {
        ...
        if (
          !mergedConfig.persistent &&
          (!isPointerDown.current || mergedConfig.terminateOnPointerUp)
        ) {
          rippleRef.fadeOut();
        }
  ...
  useEffect(() => {
    ...
    events.forEach((event) =>
      container.addEventListener(event, handlePointerDown)
    );

    return () => {
      events.forEach((event) =>
        container.removeEventListener(event, handlePointerDown)
      );
    };
  }, [handlePointerDown]);
  ...
  useEffect(() => {
    ...
    return () => {
      events.forEach((event) =>
        window.removeEventListener(event, handlePointerUp)
      );
    };
  }, [handlePointerUp]);
